...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    return ceil((daily_rate(hourly_rate)*22)-((discount/100)*(daily_rate(hourly_rate)*22)));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    return floor(budget/((daily_rate(hourly_rate)-((discount/100)*(daily_rate(hourly_rate))))));
}
